...
private :
    ...
    /// Coef multiplicateur de influence (m_weight)
    double Coef_mult;

    /// booléen pour savoir si l'influence est positive ou négative
    bool negpos;
...
private :
    ...
    std::vector<std::vector<int>> k_plets_solutions;
    std::vector<int> m_combinaison;
    std::vector<std::vector<int>> toutes_combinaisons;
...
public:
    ...
    void SAUVEGARDER_GRAPH (std::string fic1, std::string fic2);
    ...
    void afficher_solutions(int i);

    int trouver_k_plets_solutions();
    void chercher_combinaisons(int offset, int k, std::vector<int> sommets_actifs);
